...
async function compile(options) {
    ...
    tsconfig.compilerOptions.declarationDir = output;
    ...
    for (let i = 0; i < all_source_files.length; i++) {
        ...
        processed_files[file_name] = Preprocessing.preprocess(file_name, source_file.text, { macros: true });
...
async function bundle(options) {
    const { entry_paths, platform = 'browser', format = 'iife', target = 'es2023', minify = false, sourcemap = "inline", error_limit = 25, } = options;
    const errors = [];
