...
class Users {
        ...
        this.server.endpoint(
            ...
            // Get user.
            {
                ...
                callback: async (stream, params) => {
                    ...
                    user.email ??= "";
                    user.password ??= "";
                    // user.phone_number ??= ""; // dont add phone number.
                    // user.created ??= "";
                    user.api_key ??= "";
                    user.support_pin ??= "";
                    // user.is_activated ??= "";
    ...
    async set_activated(uid, is_activated) {
        return await this._sys_set(uid, {is_activated: is_activated});
    }
    ...
    async set_first_name(uid, first_name) {
        const user = await this.get(uid);
        return await this._sys_set(uid, {first_name});;
    }
    ...
    async set_last_name(uid, last_name) {
        const user = await this.get(uid);
        return await this._sys_set(uid, {last_name});;
    }
    ...
    async set_username(uid, username) {
        ...
        return await this._sys_set(uid, {username});
    }
    ...
    async set_email(uid, email) {
        ...
        return await this._sys_set(uid, {email});
    }
    ...
    async set_password(uid, password) {
        return await this._sys_set(uid, {password: this.server._hmac(password)});
    }
