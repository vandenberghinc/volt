 ...
 */
const inflight_event_id_cache = new vlib.Cache<string, true>({
    max_size: 250_000,
    ttl: {
        sliding: true,
    ...
    for (const product of all_products) {
        if (product.type === "subscription") {
            ...
            throw new InternalStripeError(
                ...
                { product_type: (product as { type?: unknown }).type },
            );
        }
    }
...
async function await_event_trigger(response: (void | Promise<any>)[]): Promise<void> {
    const promises = response.filter((r): r is Promise<any> => Boolean(r));
 ...
 */
const required_stripe_webhook_events: Stripe.WebhookEndpointUpdateParams.EnabledEvent[] = [
...
/**
 * Database record for webhook endpoint persistence.
 *
 * IMPORTANT: the signing secret is sensitive. Ideally store it in a secrets manager.
 * If you must store it in DB, ensure the DB layer is encrypted at rest + access-controlled.
 */
interface StripeWebhookEndpointRecord {
    /** The webhook URL (current configured URL in Stripe). */
    webhook_url: string;
    /** The Stripe Webhook Endpoint id (we_...). */
    stripe_webhook_endpoint_id: string;
    /** The webhook signing secret (whsec_...). */
    webhook_signing_secret: string;
    /** Enabled events configured in Stripe. */
    enabled_events: Stripe.WebhookEndpointUpdateParams.EnabledEvent[];
    /** Updated timestamp (unix ms). */
    updated_at_ms: number;
    /** The webhook app id (stored in metadata). */
 ...
 */
function create_stripe_webhook_endpoints_db(server: Server): Collection<StripeWebhookEndpointRecord> {
    return server.db.collection<StripeWebhookEndpointRecord>({
 ...
 */
function assert_https_webhook_url(webhook_url: string): void {
    ...
    try {
        ...
        throw new InternalStripeError(
            "invalid_argument",
            "Property 'webhook_url' must be a valid absolute URL.",
            { webhook_url },
            error,
        );
    }
 ...
 */
function same_event_set(a: ReadonlyArray<string>, b: ReadonlyArray<string>): boolean {
    if (a.length !== b.length) return false;
    const set_a = new Set(a);
    if (set_a.size !== b.length) return false;
    for (const ev of b) {
        if (!set_a.has(ev)) return false;
    }
    return true;
}
 ...
 */
function extract_webhook_secret(endpoint: Stripe.WebhookEndpoint): string | undefined {
 ...
 */
async function find_stripe_webhook_by_app_id(
    client: Stripe,
    webhook_app_id: string,
    ...
    for (;;) {
        const page = await stripe_api_call(
            () => client.webhookEndpoints.list({ limit: 100, ...(starting_after ? { starting_after } : {}) }),
            { operation: "webhookEndpoints.list", webhook_app_id },
        );
 ...
 */
export async function register_or_update_stripe_webhook_endpoint(
    client: Stripe,
    server: Server,
    opts: {
    ...
    const loaded = await webhook_endpoints_db.load(
    ...
    if (!(loaded instanceof Error)) {
        ...
        try {
            endpoint = await stripe_api_call(
                () => client.webhookEndpoints.retrieve(loaded.stripe_webhook_endpoint_id),
                {
        ...
        if (endpoint.metadata?.[webhook_metadata_app_id] !== opts.webhook_app_id) {
            ...
            if (!by_app_id) {
                throw new InternalStripeError(
                    ...
                    "Webhook endpoint app id does not match and no endpoint was found for webhook_app_id.",
                    { webhook_url: opts.webhook_url, webhook_app_id: opts.webhook_app_id },
                );
            }
            ...
            throw new InternalStripeError(
                ...
                "Stripe webhook endpoint for webhook_app_id exists but signing secret is not available via DB lookup. Store the whsec_ value at creation time.",
                { webhook_url: opts.webhook_url, stripe_webhook_endpoint_id: by_app_id.id, webhook_app_id: opts.webhook_app_id },
            );
        }
        ...
        if (needs_event_update || needs_enable) {
            ...
            const update_params: Stripe.WebhookEndpointUpdateParams = {
                enabled_events,
        ...
        const updated_record: StripeWebhookEndpointRecord = {
            webhook_url: opts.webhook_url,
            stripe_webhook_endpoint_id: loaded.stripe_webhook_endpoint_id,
        ...
        await webhook_endpoints_db.set(
            { webhook_app_id: opts.webhook_app_id },
            updated_record,
            { throw: true, retry: 3 },
        );
    ...
    if (loaded instanceof Error && !(loaded instanceof Collection.NotFoundError)) {
        throw new InternalStripeError(
            ...
            "Failed to access webhook endpoints record.",
            { webhook_url: opts.webhook_url },
            loaded,
        );
    }
    ...
    if (existing) {
        ...
        if (needs_event_update || needs_enable || needs_url_update) {
            ...
            await stripe_api_call(
                () =>
                    client.webhookEndpoints.update(existing.id, update_params, {
                        idempotencyKey: stable_idempotency_key(
                            `webhook_endpoints.update:${existing.id}:${opts.webhook_url}`,
                            255,
                        ),
                    }),
                {
        ...
        throw new InternalStripeError(
            "webhook_endpoint_secret_missing",
            "Stripe webhook endpoint exists but signing secret is not available. Store the whsec_ value at creation time.",
            { webhook_url: opts.webhook_url, stripe_webhook_endpoint_id: existing.id },
        );
    }
    ...
    const create_params: Stripe.WebhookEndpointCreateParams = {
    ...
    const created = await stripe_api_call(
        () =>
            client.webhookEndpoints.create(create_params, {
                // Stable idempotency ensures safe retries without creating multiple endpoints.
                idempotencyKey: stable_idempotency_key(
                    `webhook_endpoints.create:${opts.webhook_app_id}:${opts.webhook_url}`,
                    255,
                ),
            }),
    ...
    if (!webhook_signing_secret) {
        ...
        throw new InternalStripeError(
            "webhook_endpoint_secret_missing",
            "Stripe did not return a webhook signing secret on endpoint creation.",
            { webhook_url: opts.webhook_url, stripe_webhook_endpoint_id: created.id },
        );
    }
        ...
        await stripe_api_call(
            () =>
                client.webhookEndpoints.update(
                    ...
                    {
                        idempotencyKey: stable_idempotency_key(
                            `webhook_endpoints.enable:${created.id}:${opts.webhook_url}`,
                            255,
                        ),
                    },
                ),
    ...
    const record: StripeWebhookEndpointRecord = {
        webhook_url: opts.webhook_url,
        stripe_webhook_endpoint_id: created.id,
        ...
        try {
            ...
            switch (event.type) {
                case "checkout.session.completed": {
                    ...
                    const mode: "payment" | "subscription" | "setup" =
                ...
                case "customer.subscription.deleted": {
                    ...
                    if (event.type === "customer.subscription.created"
                        ...
                        await enforce_single_subscription_plan(
                            client,
                            opts.server,
                            {
                                uid,
