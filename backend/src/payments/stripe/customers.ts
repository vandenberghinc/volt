 ...
 */
function assert_uid(uid: string): void {
    if (typeof uid !== "string" || uid.length < 16 || uid.length > 128) {
        throw new InternalStripeError(
            "invalid_uid",
            "Invalid uid for Stripe customer mapping.",
            { uid_length: typeof uid === "string" ? uid.length : null },
        );
    }
    ...
    if (/[\u0000-\u001F\u007F]/.test(uid)) {
        throw new InternalStripeError(
            "invalid_uid",
            "uid contains control characters.",
            {},
        );
    }
}
 ...
 */
function create_customer_db(server: Server): Collection<CustomerRecord> {
    // Initialize the database connection.
    return server.db.collection<CustomerRecord>({
 ...
 */
async function find_stripe_customer_id(
    client: Stripe,
    uid: string,
): Promise<string | null> {
    assert_uid(uid);
...
export async function ensure_stripe_customer(
    ...
    // Not found.
        ...
        const final_record = await db.load(
            { uid },
            {
                throw: false,
                projection: { customer_id: 1 },
        ...
        if (final_record instanceof Error) {
            throw new InternalStripeError(
                "customer_not_found",
                "Failed to load Stripe customer from database after creation.",
                { uid, cause: final_record },
            );
        }
    ...
    // Error.
        ...
        throw new InternalStripeError(
            "customer_not_found",
            "Failed to load Stripe customer from database.",
            { uid, cause: record },
        );
    }
...
export async function delete_stripe_customer(
    ...
    const deleted_customer = await stripe_api_call(
        () => client.customers.del(record.customer_id),
        { uid, customer_id: record.customer_id }
    );
