...
export class Collection<Data extends mongodb.Document = mongodb.Document> {
    ...
    private _index_key_signature(keys: Record<string, number>): string {
        // Preserve order (Mongo treats order as significant for compound indexes)
        return Object.entries(keys).map(([k, v]) => `${k}:${v}`).join("|");
    }

    private _keys_equal(a: Record<string, number>, b: Record<string, number>): boolean {
        const aEnt = Object.entries(a);
        const bEnt = Object.entries(b);
        if (aEnt.length !== bEnt.length) return false;
    ...
    private _normalize_index_opts(
        opts: string | Collection.IndexOpts
    ): {
        keys_obj: Record<string, number>;
        ...
        if (typeof opts === "string") {
            ...
            const user_options = opts.options as unknown as undefined | mongodb.CreateIndexesOptions;
     ...
     */
    private async _drop_non_init_indexes(): Promise<void> {
        this.assert_not_transaction_based();
        this.assert_init();
        ...
        for (const ix of existing) {
            ...
            const keyObj = (ix as any).key as Record<string, number> | undefined;
            if (!keyObj) continue;

            const sig = this._index_key_signature(keyObj);

            if (desired.has(sig)) continue;
            if (keep_ttl && sig === ttl_sig) continue;
     ...
     */
    private async _create_index(opts: string | Collection.IndexOpts): Promise<string> {
        this.assert_not_transaction_based();
        if (!this.initialized) { await this.init(); } this.assert_init();
        ...
        try {
            try {
                ...
                if (forced && err && typeof err === "object" && (err as any).codeName === "IndexKeySpecsConflict") {
                    await drop_index();
                    return await this._col.createIndex(keys_obj, options);
                }
                throw err;
