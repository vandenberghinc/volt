...
import { AggregationCursor, Collection as MongoCollection, ObjectId, Document as MongoDoc } from 'mongodb';
 ...
 * });
 */
export class Collection<Data extends Document.Data = any> {
    ...
    private _init_indexes?: (string | IndexOptions)[];
    ...
    assert_init(): asserts this is { _col: MongoCollection } {
        if (this._col == null) {
    ...
    private _process_doc(doc: NonNullable<any>): Data;
    private _process_doc(doc: any): Data | undefined {
        if (doc == null) { return ; }
    ...
    async col(): Promise<NonNullable<MongoCollection>> {
        await this.init();
        return this._col!;
    }
    ...
    ref<Data extends Document.Data = any>(query: Query, opts?: Omit<ConstructorParameters<typeof Document.Ref<Data>>[1], "col">): Document.Ref<Data> {
        return new Document.Ref<Data>(query, opts ? { ...opts, col: this } : { col: this });
    }
     ...
     */
    async create_index(opts: string | IndexOptions): Promise<string> {
        if (!this.initialized) { await this.init(); } this.assert_init();
        let key: string | undefined, keys: string[] | undefined
        let options: IndexOptions["options"] | undefined
        let forced = false;
     ...
     *      @type: object
     */
    async find(query: Record<string, any>): Promise<Data | undefined> {
        if (!this.initialized) { await this.init(); } this.assert_init();
        try {
    ...
    async find_many(query: Record<string, any>): Promise<Data[] | undefined> {
        if (!this.initialized) { await this.init(); } this.assert_init();
        try {
     ...
     *      @type: string
     */
    async exists(path: Query): Promise<boolean> {
        if (!this.initialized) { await this.init(); } this.assert_init();
    ...
    /** Update many. */
    async update_many(...args: Parameters<typeof MongoCollection.prototype.updateMany>): Promise<any> {
        if (!this.initialized) { await this.init(); } this.assert_init();
    ...
    async list(
        ...
        try {
            const docs = await this._col.find(path, {projection: options.projection}).toArray() as Data[];
    ...
    async list_query(
        ...
        if (!this.initialized) { await this.init(); } this.assert_init();
        try {
            const docs = await this._col.find(query, {projection: options.projection}).toArray() as Data[];
    ...
    async list_all(
        ...
        if (!this.initialized) { await this.init(); } this.assert_init();
        let docs: Data[];
        docs = await this._col.find(query, {projection: options.projection}).toArray() as Data[];
        if (options.process === false) {
            return docs;
        }
        return docs.map((doc) => this._process_doc(doc)) // list as array since the user might have used a path object width different attributes so dict is not reliable.
     ...
     *      @type: object
     */
    async delete_query(query: Record<string, any> = {}): Promise<void> {
        if (!this.initialized) { await this.init(); } this.assert_init();
    ...
    // Delete all.
    async delete_all(path: Query): Promise<void> {
        if (!this.initialized) { await this.init(); } this.assert_init();
        if (typeof path !== "string" && (typeof path !== "object" || path == null)) {
            throw Error(`Parameter "path" has an invalid type "${typeof path}", the valid type is "string".`);
        }
    ...
    /** Aggregate */
    async aggregate<T extends Partial<Data> = Data>(pipeline: MongoDoc[], opts: { cursor?: false, clean?: boolean }): Promise<T[]>;
    async aggregate<T extends Partial<Data> = Data>(pipeline: MongoDoc[], opts?: { cursor: true, clean?: boolean }): Promise<AggregationCursor<T>>;
    async aggregate<T extends Partial<Data> = Data>(pipeline: MongoDoc[], opts?: { cursor?: boolean, clean?: boolean }): Promise<T[] | AggregationCursor<T>> {
        if (!this.initialized) { await this.init() } this.assert_init();
        const out = this._col.aggregate<T>(pipeline);
