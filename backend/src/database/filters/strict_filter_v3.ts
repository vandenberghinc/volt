...
/* ------------------------------- $expr typing ----------------------------- */
// Strict field references - only allow valid paths from WithId<T>
// Need to generate all valid field paths including _id
...
export type Expr<T extends MongoDocument> =
    ...
    | {
        ...
        $concat?: Expr<T>[];
        $size?: Expr<T>;
        $in?: [Expr<T>, Expr<T>]; // [needle, haystack/array-expr]
...
// Helper to properly distribute arrays over union types
// This ensures that for a union type T = 'a' | 'b' | 'c',
// we get ('a' | 'b' | 'c')[] which allows any subset like ['a', 'b']
...
export type FieldOperators<T> = EqualityOps<Defined<T>> &
