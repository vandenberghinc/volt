...
import type { Document as MongoDocument, Filter as MongoFilter, Collection } from 'mongodb';
type KnownKeys<T> = {
...
export type FieldOperators<T> = EqualityOps<Defined<T>> & ExistsAndType & NumericComparators<Defined<T>> & StringOps<Defined<T>> & ArrayOps<T extends undefined ? never : T> & BitwiseOps<Defined<T>> & {
 ...
 */
export type StrictFilter<TSchema extends MongoDocument> = any;
/** Helper to show compatibility with the official driver types. */
export type IsAssignableToMongoFilter<T extends MongoDocument> = StrictFilter<T> extends MongoFilter<T> ? true : false;
export declare class SomeClass<Data extends MongoDocument> {
    col: Collection<Data>;
    cast(query: StrictFilter<Data>): void;
    find(query: StrictFilter<Data>): Promise<mongodb.WithId<Data> | undefined>;
}
export {};
