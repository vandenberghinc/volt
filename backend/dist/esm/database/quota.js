...
(function (QuotaManager) {
        ...
        (function (Opts) {
            ...
            function validate(quota) {
                ...
                if ((e = Query.validate(quota)))
                    return e;
                if ((e = Quota.Opts.validate(quota)))
                    return e;
            }
            Opts.validate = validate;
    ...
    // ----------------------------------------------------------------
    /**
     * AmountScale represents the unit scale for cost/usage values.
     * It is an enum to provide a single source of truth for both type and value.
     *
     * @example
     * - Currency: Base = 1 (whole units), Nano = 1_000_000_000 (nano units).
     * - Generic units: Base = unit, Nano = nano-units.
     */
    let AmountScale;
    (function (AmountScale) {
        /** Whole-unit scale (1). */
        AmountScale[AmountScale["Base"] = 1] = "Base";
        /** Nano-unit scale (1e9). */
        AmountScale[AmountScale["Nano"] = 1000000000] = "Nano";
