...
export interface TabElement extends HStackElement {
    ...
    _on_select?: ((TabElement) => any);
    _on_unselect?: ((TabElement) => any);

    on_select(): undefined | ((TabElement) => any);
    on_select(callback: ((TabElement) => any)): this;

    on_unselect(): undefined | ((TabElement) => any);
    on_unselect(callback: ((TabElement) => any)): this;
...
export class TabsElement extends (VStackElement as any as VElementBaseSignature) {
    ...
    public _tab_nodes: (HStackElement & TabElement)[];
    public _on_tab_header?: OnTabHeader;
    public _div?: DividerElement;
     ...
     */
    // Build.
    build(content: TabContentItem[] | Record<string, any> = []) {
        ...
        if (content && typeof content === "object" && !Array.isArray(content)) {
            ...
            console.error(`Invalid parameter type "${Schema.value_type(content)}" for parameter "content", the valid type is "array" or "object".`)
            return this;
        }
        content = content as { title: string, content: any }[]
        ...
        this.append(
            HStack(
                ForEach(content as TabContentItem[], (content_item, index, is_last) => {
                    const name = content_item.title;
                    const content_stack = content_item.content;
                    if (typeof name !== "string") {
                        console.error(`Invalid parameter type "${Schema.value_type(name)}" for parameter "content.${index}.title", the valid type is "string".`)
                    } else if (content_stack == null) {
                        console.error(`Invalid parameter type "${Schema.value_type(content_stack)}" for parameter "content.${index}.content", the valid type is "Node". or "Array"`)
                    }
                    const item = HStack<TabElement>( // use an hstack so the user can still add flexed elements through the on_tab_header callback.
                        Text(name)
                        ...
                        } as HStackElement & TabElement)
                        .on_mouse_over(e => {
                            if (e !== main_this._selected_node) {
                                if (this._tab_opac !== -1) {
                                    e.opacity(1)
                                }
                                const color = window.getComputedStyle(e as any).color;
    ...
    /**
     * Select a tab based on the title text.
     * @docs
    */
    // @ts-ignore
    ...
    /**
     * Set or get the tab title opacity. By default non selected tabs have a slight opacity `0.8`, value `-1` can be used to disable this behaviour.
     * @docs
     */
    ...
    /**
     * Set or get the divider background color.
     * @docs
     */
    ...
    /**
     * Set or get the divider opacity from mouse over events. By default in mouse over events the tab bottom divider will have the same color as the tab title, but with an opacity.
     * @docs
     */
     ...
     * @param callback The callback accepts parameters `(tab_name, tab_node, node)`. The last node is the parent `TabsElements` object.
     * @docs
     */
...
declare module './any_element.d.ts' { interface AnyElementMap { TabsElement: TabsElement } }
